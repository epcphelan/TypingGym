...
class TypeAssistKey extends Component {
  static isActive(activeChars, lowChar, shiftChar) {
    return (activeChars.indexOf(lowChar) > -1 || activeChars.indexOf(shiftChar) > -1);
  }
  shouldComponentUpdate(nextProps) {
    return (TypeAssistKey.isActive(this.props.activeChars, this.props.lowChar, this.props.shiftChar)
      !== TypeAssistKey.isActive(nextProps.activeChars, nextProps.lowChar, nextProps.shiftChar))
  ...
  render() {
    ...
    const styles = {
      backgroundColor: `rgba(255,0,0,${Math.min(this.props.errorHeatMapScore, 0.5)})`,
    };
    let keyContents = [
      (<div key="upper-char" className="shift-char"> {this.props.shiftChar} </div>),
    ];
    if (!isUpperLowerPair) {
      keyContents.push(<div key="lower-char" className="low-char"> {this.props.lowChar} </div>);
    }
    return (
      <div style={styles} className={classNames}>{keyContents}</div>
    );
  }
}
